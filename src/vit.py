...
# --- Add this import near the top of vit.py, alongside torch/numpy ---
from zennit.composites import LayerMapComposite
import zennit.rules as z_rules

# --- AttnLRP support (lxt "efficient" mode) ---
#
# COMPATIBILITY SHIM: lxt.efficient unconditionally imports its BERT
# support module (lxt/efficient/models/bert.py), which does:
#     from transformers.pytorch_utils import find_pruneable_heads_and_indices
# Recent `transformers` releases removed this function from pytorch_utils,
# which crashes the import even though we only ever use ViT, not BERT.
# The function's implementation has been stable across transformers
# versions for years, so we simply restore it in-place before lxt is
# imported. Safe to remove this block once you upgrade to an lxt release
# that no longer depends on the old location.
...
if not hasattr(_hf_pytorch_utils, "find_pruneable_heads_and_indices"):
    ...
    _hf_pytorch_utils.find_pruneable_heads_and_indices = _find_pruneable_heads_and_indices

# This patches the HF ViT modeling module in-place so that softmax and
# matmul operations inside self-attention propagate LRP relevance via a
# standard backward pass (Input*Gradient formulation of AttnLRP), instead
# of only the Gamma-rule on nn.Linear layers used by lrp_forward_pass.
from transformers.models.vit import modeling_vit
from lxt.efficient import monkey_patch as lxt_monkey_patch

_ATTNLRP_PATCHED = False  # module-level guard so we only patch once
